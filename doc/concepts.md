...
|                             | `MsgDocumentReader`     | `get_documentreader` `get_live_document`                                | Live configuration documents that auto-update when republished. `get_live_document` returns dict-like `LiveDocument` for immediate use. Supports both dict and namespace-style access (`doc['key']` or `doc.key`).                                             |
