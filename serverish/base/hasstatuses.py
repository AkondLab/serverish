...
class HasStatuses(Manageable):
    ...
    def __init__(self, name: str = None, parent: Collector = None, **kwargs) -> None:
        self.check_methods: dict[str, Callable[[], Awaitable[Status]]] = {'main': self.diagnose_dummy_ok}

        super().__init__(name, parent, **kwargs)

    def set_check_methods(self, **methods: Callable[[], Awaitable[Status]]) -> None:
        """Sets check methods
        ...
        Args:
            **methods (dict[str, Callable]): Dict of asynchronous methods
        """
        self.check_methods = methods

    def add_check_methods(self, at_beginning: bool,
                          **methods: Callable[[], Awaitable[Status]]) -> None:
        """Adds check methods
    ...
    async def diagnose(self, no_deduce: bool = False) -> dict[str, Status]:
        ...
        """
        if no_deduce:
            checking_tasks = [m() for m in self.check_methods.values()]
