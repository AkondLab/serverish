...
@pytest.mark.nats
async def test_messenger_pub_simple(messenger, unique_subject):
...
@pytest.mark.nats
async def test_messenger_pub_sub(messenger, unique_subject):
...
@pytest.mark.nats
async def test_messenger_pub_then_sub(messenger, unique_subject):
    subject = unique_subject
...
@pytest.mark.nats
async def test_messenger_pub_sub_pub(messenger, unique_subject):
    ...
    pub = get_publisher(unique_subject)
    sub = get_reader(unique_subject, deliver_policy='all')
...
@pytest.mark.nats
async def test_messenger_big_pub_small_sub(messenger, unique_subject):
...
@pytest.mark.nats
async def test_messenger_immediate_message_delivery(messenger, unique_subject):
...
@pytest.mark.nats
async def test_messenger_pub_time_pub_sub(messenger, unique_subject):
    ...
    sub = get_reader(unique_subject, deliver_policy='by_start_time', opt_start_time=now)
    await subsciber_task(sub)
    await pub.close()
    await sub.close()
...
@pytest.mark.nats
async def test_messenger_scheduled_open(nats_server):
