...
async def test_messenger_progress():
    subject = 'test.messenger.test_messenger_progress'
    ...
    sub = get_callbacksubscriber(subject, deliver_policy='all')
    ...
    async with Messenger().context(host='localhost', port=4222) as mess:
        async with sub:
            await mess.purge('test.messenger.test_messenger_progress')
